...
>
> **BS-3 — `NEXT_PUBLIC_API_BASE_URL` is intentionally empty**: Traefik routes `/api/` directly to the .NET API, so `NEXT_PUBLIC_API_BASE_URL` is empty in all deployed environments. `EventSource` must connect via the relative path `/api/stream` when the env var is absent — this is the normal production case, not an error. Do NOT skip the connection or warn when the env var is unset. When the env var IS set (e.g. local dev pointing at a direct API port), use it as the origin prefix.
>
...
- [x] 2. Add Traefik SSE middleware to `docker/compose/traefik_dynamic.yml`
...
- [x] 3. Implement `SseConnectionManager` singleton
...
- [x] 4. Implement `IScheduleEventPublisher` and `SseEventPublisher`
...
- [x] 5. Implement `StreamController`
...
- [x] 6. Inject `IScheduleEventPublisher` into `ScheduleService`
...
- [x] 7. Publish vote events from `DiscoveryService` / `DiscoveryController`
...
- [x] 8. Publish `recipe_ready` from `RecipeReadyProcessor`
  - After marking recipe as ready, publish `RecipeReadyEvent { recipeId }`
  - Integration test: `RecipeReady_PublishesRecipeReadyEvent`

- [x] 9. Run `dotnet test` — all backend tests pass

---

### Group D — Frontend: Store Changes

- [x] 10. Add `applyServerUpdate` and `clearOptimisticGuard` to `todayStore`
...
- [x] 10b. Extend `discoveryStore` with fill-the-gap invalidation signal (BS-7)
...
- [x] 11. Add `applySnapshot`, `applySlotUpdate`, `applyVoteUpdate`, `applySmartDefaultsUpdate` to `weekStore`
...
- [x] 12. Implement `useScheduleStream` hook
  - File: `pwa/src/hooks/useScheduleStream.ts`
  - **URL construction (BS-3 revised)**: `NEXT_PUBLIC_API_BASE_URL` is intentionally empty in all deployed environments — Traefik routes `/api/` directly. When the env var is absent, connect to the relative path `/api/stream`. When it is set (local dev with a direct API port), use `${NEXT_PUBLIC_API_BASE_URL}/api/stream`. Do NOT skip the connection or warn when the env var is unset.
  - Creates `EventSource` to the resolved URL with `{ withCredentials: true }` — cookies (`x-family-member-id`, `h_access`) are sent automatically (BS-4)
...
- [x] 13. Mount `useScheduleStream` at app layout level
...
- [x] 13b. Pre-seed `weekStore` at layout level for `weekOffset=0` (BS-2)
...
- [x] 14. Move `todayStore` initialisation to layout level; remove duplicate `init()` from `HomeCommandCenter`
...
- [x] 14b. Wire `QuickFindModal` to `discoveryStore.fillTheGapVersion` (BS-7)
...
- [x] 15. Add SSE mock to `setupCommonRoutes` in `pwa/e2e/mock-api.ts`
...
- [x] 16. E2E: SSE `slot_updated` → `TonightMenuCard` appears on home without navigation
...
- [x] 17. E2E: SSE `slot_updated` → planner day card updates
...
- [x] 18. E2E: SSE `recipe_ready` → GOTO confirm button appears
...
- [x] 19. Re-enable skipped test: Planner Quick Find for today → home shows TonightMenuCard
...
- [x] 20. Remove 30-second poll from `useWeekStore`
...
- [x] 21. Remove GOTO synthesis polling from `HomeCommandCenter`
...
- [x] 22. Run `task agent:drift` — zero schema drift
- [x] 23. Run `dotnet test` — all 147+ backend tests pass
- [x] 24. Run `task review` — formatting, linting, typecheck, all E2E tests pass
...
- [x] 25. Add `recipe_failed` SSE event to `IScheduleEventPublisher`
...
- [x] 26. Extend `recipe_ready` payload with name and imageUrl
...
- [x] 28. Create `libraryStore` (notification queue)
...
- [x] 29. Extend `discoveryStore` with live stack management
  - File: `pwa/src/store/discoveryStore.ts`
...
- [x] 30. Extend `useScheduleStream` to handle new event types
  - File: `pwa/src/hooks/useScheduleStream.ts`
...
- [x] 31. Update `MinimalCapture` success screen — honest async state
...
- [x] 32. Create notification components — `LibraryToast` and `RecipeFailureBanner`
...
- [x] 33. Update Discovery page to use `discoveryStore.discoveryStack`
...
- [x] 34. Add family interest ring to `DiscoveryCard`
...
- [x] 34b. Add processing indicator dot to Capture nav icon
...
- [x] 35b. Fix the OpenAPI contract — add `groceryState` to `ScheduleDays`
...
- [x] 35c. Add `groceryState` to `.NET` `ScheduleDays` DTO and `GetScheduleAsync`
...
- [x] 35d. Remove `additionalData` grocery hack from `weekStore`
  - File: `pwa/src/store/weekStore.ts`
...
- [x] 35e. Add `grocery_updated` SSE event to backend
...
- [x] 35f. Handle `grocery_updated` in `useScheduleStream`
  - File: `pwa/src/hooks/useScheduleStream.ts`
  - Add handler:
    ```typescript
...
- [x] 35g. Remove `isSaving` global spinner from `GroceryList`
...
- [x] 35h. Add SSE mock for grocery to `mock-api.ts`
...
- [x] 38. Wire GOTO settings card to `recipe_ready` SSE event
...
- [x] 39. Fix Profile page — title, contrast, and dead-end
...
- [x] 40. Fix Settings page — contrast and section headers
  ...
  - **GOTO card pending state** (in `FamilyGOTOSettings.tsx`):
    - Add `"Usually ready in under 60 seconds"` subtitle below spinner (see Task 38)
...
- [x] 41. Database schema audit and drift check
...
- [x] 42. Build missing flow documents
...
- [x] 43. E2E test coverage audit and missing tests
...
- [x] 45. Ingredient pre-flight checklist — Card 0 interactivity
...
- [x] 46. Remove dietary badge from Card 0
  - File: `pwa/src/components/planner/CooksMode.tsx`
...
- [x] 47. Card 0 zone differentiation and ingredient contrast
  - File: `pwa/src/components/planner/CooksMode.tsx`
...
- [x] 48. Step instruction readability fix (Cards 1–N)
  - File: `pwa/src/components/planner/CooksMode.tsx`
...
- [x] 49. Navigation button proportion fix
  - File: `pwa/src/components/planner/CooksMode.tsx`
...
- [x] 50. Done moment — micro-celebration
  - File: `pwa/src/components/planner/CooksMode.tsx`
...
- [x] 35. Add new SSE event schemas to `specs/openapi.yaml`
...
- [x] 36. Run `dotnet test` — all backend tests pass including new event publisher tests
- [x] 37. Run `task review` — all E2E tests pass including:
