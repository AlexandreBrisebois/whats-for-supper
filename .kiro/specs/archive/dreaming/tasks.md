...
- [x] 1. **Engine - Support Scheduled Triggers**
...
- [x] 2. **Test - Scheduled Trigger Unit Tests**
...
- [x] 3. **Engine - Implement `StartWorkflow` Processor**
...
- [x] 4. **Test - Cron & Spawn Workflow Unit Tests**
...
- [x] 5. **Management - Create Dreaming Workflow YAML**
...
- [x] 6. **Management - Implement Pruning & Reporting**
...
- [x] 7. **Test - Maintenance Logic Tests**
...
- [x] 8. **Infrastructure - Initial Dreaming Seeder**
...
- [x] 9. **Verification - Full Cycle Execution**
...
- [x] 10. **Documentation - Update Project Assets**
    - [x] 10.1 Update `README.md` with maintenance and backup details.
    - [x] 10.2 Update deployment documentation with `DREAMING_CRON_UTC`.
    - [x] 10.3 Update `HANDOVER.md` with maintenance cycle logic.
    - _Requirements: All_

## Notes / Decisions

- 2026-05-09: Implemented recurring scheduling through task-level `WorkflowTask.ScheduledAt`; no `WorkflowInstance.ScheduledAt` column was added.
- 2026-05-09: `StartWorkflow` is the only engine-level spawn processor. It supports immediate child workflow starts and scheduled child starts using `payload.schedule.cron`.
- 2026-05-09: Dreaming uses `DREAMING_CRON_UTC` with default `0 3 * * *`; cron is interpreted in UTC.
- 2026-05-09: The startup seeder creates an initial future `dreaming` instance only when no `dreaming` instance is pending or processing.
