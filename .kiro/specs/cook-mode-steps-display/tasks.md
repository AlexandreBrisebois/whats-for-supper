...
- [ ] 1. Write bug condition exploration test
  - **Property 1: Bug Condition** - HowToSection[] Input Rejected by TypeScript
  - **CRITICAL**: This test MUST FAIL (or fail to compile) on unfixed code — failure confirms the bug exists
  - **DO NOT attempt to fix the test or the code when it fails**
  - **NOTE**: This test encodes the expected behavior — it will validate the fix when it passes after implementation
  - **GOAL**: Surface the counterexample that demonstrates the bug: the `as any` cast in `step-parser.spec.ts` line 41 is the documented counterexample
  - **Scoped PBT Approach**: The bug is deterministic (any `HowToSection[]` input triggers it), so scope the property to the concrete failing cases from the design
  ...
  - Observe the TypeScript error: `Argument of type '{ "@type": string; name: string; itemListElement: ...; }[]' is not assignable to parameter of type 'string[] | { name?: string; text?: string; }[]'`
  ...
  - Run `task gate` on UNFIXED code — expect TypeScript compilation FAILURE (this is correct — it proves the bug exists)
  - Document counterexample: `parseRecipeSteps([{ '@type': 'HowToSection', name: 'Spaghetti Preparation', itemListElement: [...] }])` — TypeScript rejects the call without `as any`
  - Mark task complete when test is written, run, and the TypeScript error is documented
...
- [ ] 3. Fix: widen type signatures and update mock data

  - [ ] 3.1 Implement the type fix — two type declaration changes, zero logic changes
  ...
  - [ ] 3.2 Update mock data to use real HowToSection[] shape
  ...
  - [ ] 3.3 Add e2e cook mode test with HowToSection recipe
