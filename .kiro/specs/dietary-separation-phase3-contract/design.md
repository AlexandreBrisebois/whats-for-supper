...
Create a reviewable preflight/evidence record and a classified live-reference manifest before altering the database. Trace source references from the PWA/API contract through controller/service/model/storage and include the supplied vocabulary plus discovery views and backup sidecars. Run the required targeted regressions against the actual supported environment. This slice creates no destructive migration and has one output: an evidenced GO/NO-GO decision.

### T1 — public-contract to live-consumer tracer

With T0=GO and the client policy recorded, remove retired fields through the whole contract seam in one atomic change: OpenAPI schemas/fields, API DTOs and mappers, search/discovery projections, runtime consumers/DI/workers, PWA generated client and handwritten adapters/store/views, fixtures/mocks, and health-only tests. Regenerate Kiota rather than hand-edit generated models.

Physical storage is intentionally still present during T1, but no runtime path may read or write it. This makes T1 independently testable and creates the zero-reader/zero-writer evidence required by T3. It must not hide a retained database property with `JsonIgnore`; the property/model and every live projection are owned by T3's final persistence cleanup.

### T2 — recovery and reindex tracer

Before irreversible storage deletion, prove the final persistence boundary in the application: new backup omits retired fields; a representative old `recipe.info` with those fields restores retained facts while unknown fields are ignored; current backup round trip works; and a clean reindex/search document build derives only WFS-owned facts. Include French vegetarian coverage, Find Similar, and planner-to-grocery mutation coverage. Failure is a NO-GO for T3.

### T3 — populated upgrade tracer

After T1/T2 and a fresh zero-reader/zero-writer audit, change the actual schema upgrade artifact(s), `schema.sql`, compatibility/install logic, DbContext, views and schema-integrity tests together. Drop health tables first only after their dependencies are removed; then drop obsolete core columns and recreate the discovery view without health projections. Inspect dependent indexes, constraints, sequences, triggers, functions, JSONB indexes, and materialized views rather than assuming the named columns are isolated.

Use a representative populated pre-Phase-3 schema and the clean full chain. Record exact dropped objects and whether downgrade is structurally incapable of recovering health data. Retained vegetarian facts/classifier metadata are asserted before and after upgrade.
...
| --- | --- |
| A T0 condition/regression is absent or fails | Stop before T1/T3 destructive work; report failed, blocked, or not-run evidence exactly. |
