...
- [ ] 1.1 Add `ingredient_categories` table to `api/database/schema.sql`
...
- [ ] 1.2 Add `grocery_items` column to `weekly_plans` in `api/database/schema.sql`
  - Add `grocery_items jsonb NOT NULL DEFAULT '[]'::jsonb` to the `weekly_plans` table definition
  - Verify: `task migrate` applies cleanly

- [ ] 1.3 Add `IngredientCategory` EF entity and update `WeeklyPlan` model
...
- [ ] 2.1 Add `GroceryLineItemDto` schema to `specs/openapi.yaml`
...
- [ ] 2.2 Add `groceryItems` to `ScheduleDays` schema in `specs/openapi.yaml`
...
- [ ] 2.3 Add `GroceryLineItemDto.cs` C# record
...
- [ ] 2.4 Update `ScheduleDays.cs` to include `GroceryItems`
...
- [ ] 2.5 Regenerate Kiota TypeScript client
  - Run `task gen:client`
...
- [ ] 3.1 Implement `IngredientNormalizer` static class
...
- [ ] 3.2 Write property-based and unit tests for `IngredientNormalizer`
...
- [ ] 3.3 Implement `AisleMapper` class and `GrocerySection` enum
...
- [ ] 3.4 Write property-based and unit tests for `AisleMapper`
...
- [ ] 3.5 * Add French keyword sets to `AisleMapper` (optional)
...
- [ ] 4.1 Implement `GroceryRecomputeService`
...
- [ ] 4.2 Write property-based and unit tests for `GroceryRecomputeService`
...
- [ ] 5.1 Call `GroceryRecomputeService` from `ScheduleService.AssignRecipeAsync`
...
- [ ] 5.2 Call `GroceryRecomputeService` from `ScheduleService.RemoveRecipeAsync`
...
- [ ] 5.3 Update `ScheduleService.GetScheduleAsync` to return `groceryItems`
...
- [ ] 6.1 Call `GroceryRecomputeService` from `SyncRecipeProcessor`
...
- [ ] 7.1 Update `aisleMapper.ts` to 10-section taxonomy
  - Rename `AisleSection` → `GrocerySection`
  - Expand to 9 sections: `'Produce' | 'Meat & Seafood' | 'Dairy & Eggs' | 'Frozen' | 'Bakery' | 'Pantry' | 'Beverages' | 'Deli' | 'Uncategorized'`
...
- [ ] 7.2 Write unit and property tests for updated `aisleMapper.ts`
  ...
  - Unit tests: one example per section, Uncategorized fallback, backward-compat check (old 5-section names no longer exported)
  - Verify: `task test:unit` passes

- [ ] 7.3 Update `GroceryList.tsx` to consume `groceryItems` from API
  ...
  - Update `AISLE_ORDER` to 10-section taxonomy in the correct display order
  - Update `AISLE_ICONS` to include all 10 sections
  - `groceryState` key remains `displayName` (unchanged)
  - Verify: `task typecheck` passes; `task test:unit` passes

- [ ] 7.4 Implement section completion UI in `GroceryList.tsx`
...
- [ ] 8.1 Implement `CategorizeIngredientsProcessor`
  ...
  - Send uncached names to LLM in a single batch call
  - Validate each returned section is a valid `GrocerySection` string; discard invalid entries with a warning log
  - Upsert valid results into `ingredient_categories` with `source='llm'`
  - Register processor in `Program.cs`
  - Verify: `dotnet build` passes

- [ ] 8.2 Write unit tests for `CategorizeIngredientsProcessor`
...
- [ ] 8.3 Update `recipe-import.yaml` to include `CategorizeIngredients` step
...
- [ ] 9.1 Add backup step 5 to `ManagementService.BackupAsync`
...
- [ ] 9.2 Add restore step 8 to `ManagementService.RestoreAsync`
...
- [ ] 10.1 Add data flow diagram to `api/docs/GROCERY_CATEGORIZATION.md`
...
- [ ] 10.2 Document LLM cost model in `api/docs/GROCERY_CATEGORIZATION.md`
...
- [ ] 10.3 Document the API's LLM usage in `api/docs/GROCERY_CATEGORIZATION.md` under a "LLM Integration" section
...
- [ ] 11.1 Verify end-to-end: assign recipe → GET /api/schedule returns groceryItems
