...
test.describe('Identity Persistence', () => {
  ...
  test('family member selection persists when navigating Home -> Profile -> Settings', async ({ page }) => {
    // 1. Start at Profile
  ...
  test('client-side navigation preserves family list even if server-side fetch fails', async ({ page }) => {
    ...
    await page.route('**/api/family', route => route.fulfill({
      status: 500,
