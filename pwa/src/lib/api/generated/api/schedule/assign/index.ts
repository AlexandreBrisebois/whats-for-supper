...
// @ts-ignore
import {
  createDisplacedRecipeDtoFromDiscriminatorValue,
  serializeAssignScheduleDto,
  serializeDisplacedRecipeDto,
  type AssignScheduleDto,
  type DisplacedRecipeDto,
} from '../../../models/index';
// @ts-ignore
import {
...
export interface AssignPostResponse_data extends AdditionalDataHolder, Parsable {
  /**
   * Present when a previously assigned recipe was displaced by this assignment and automatically moved to another slot (e.g. carried to next week Monday).
   */
  displacedRecipe?: DisplacedRecipeDto | null;
  /**
...
export interface AssignRequestBuilder extends BaseRequestBuilder<AssignRequestBuilder> {
  /**
   * Assigns a recipe to the specified day. If the target slot is already occupied and falls on Sunday (dayIndex 6), the displaced recipe is automatically carried forward to Monday of the following week (weekOffset+1, dayIndex 0) rather than being silently dropped.
...
export function deserializeIntoAssignPostResponse_data(
  ...
  return {
    displacedRecipe: (n) => {
      assignPostResponse_data.displacedRecipe = n.getObjectValue<DisplacedRecipeDto>(
        createDisplacedRecipeDtoFromDiscriminatorValue
      );
    },
    message: (n) => {
...
export function serializeAssignPostResponse_data(
  ...
  writer.writeObjectValue<DisplacedRecipeDto>(
    'displacedRecipe',
    assignPostResponse_data.displacedRecipe,
    serializeDisplacedRecipeDto
  );
