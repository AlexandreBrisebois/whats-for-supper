  ...
  return (
    <div className="glass px-4 py-5 flex flex-col gap-4 rounded-3xl relative overflow-hidden transition-all">
      ...
      <div className="flex flex-col gap-1 px-1">
        <h3 className="font-heading text-2xl font-medium text-[#4A3728]">
          {recipeName}
        </h3>
        {description && (
          <p className="text-[#4A3728]/80 text-sm font-medium">
            {description}
          </p>
        )}
      </div>
...
export function PrepChecklist({ tasks }: { tasks: { id: string; label: string; time?: string; completed: boolean }[] }) {
  return (
      ...
      <div className="flex flex-col gap-4 px-1">
        {tasks.map((task) => (
          <div key={task.id} className="flex items-center gap-4 group">
            <div className={`h-6 w-6 rounded flex items-center justify-center border-2 transition-colors ${task.completed ? 'bg-[#CD5D45] border-[#CD5D45] text-white' : 'border-[#4A3728]/40 bg-transparent'}`}>
              {task.completed && <Check strokeWidth={3} size={14} />}
            </div>
            <span className="flex-1 text-[#4A3728] text-base font-medium">
              {task.label}
            </span>
            {task.time && (
